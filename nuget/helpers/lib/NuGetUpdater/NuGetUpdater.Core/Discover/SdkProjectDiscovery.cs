        ...
        foreach (var tfm in tfms)
        {
            ...
            try
            {
                ...
                var (exitCode, stdOut, stdErr) = await MSBuildHelper.SidelineGlobalJsonAsync(startingProjectDirectory, repoRootPath, async () =>
                {
                    ...
                    var dependencyDiscoveryPropsPath = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location)!, "DependencyDiscovery.props");
                    var args = new string[]
                    {
                        "build",
                        startingProjectPath,
                        "/t:GenerateBuildDependencyFile",
                        $"/p:TargetFramework={tfm}",
                        $"/p:CustomAfterMicrosoftCommonCrossTargetingTargets={dependencyDiscoveryPropsPath};CustomAfterMicrosoftCommonTargets={dependencyDiscoveryPropsPath}",
                        $"/bl:{binLogPath}"
                    };
